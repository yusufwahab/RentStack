    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-semibold text-[#0F172A]">Add New Tenant</h2>
          ...
          {[
            ...
            <div key={key}>
              ...
              <input
                ...
                className="w-full border border-[#E2E8F0] rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A84C]/40"
              />
            </div>
          ))}
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-[#0F172A] text-white py-2.5 rounded-lg text-sm font-medium mt-2 hover:bg-[#1E293B] transition-colors duration-200 disabled:opacity-60"
          >
  ...
  return (
    <div className="p-6 md:p-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-[#0F172A]">Tenants</h1>
        <button
          onClick={() => setShowModal(true)}
          className="bg-[#0F172A] text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#1E293B] transition-colors duration-200"
        >
          Add Tenant
        </button>
      </div>
      ...
      {tenants && tenants.length > 0 && (
        <input
          ...
          className="w-full md:w-72 border border-[#E2E8F0] rounded-lg px-4 py-2 text-sm mb-6 focus:outline-none focus:ring-2 focus:ring-[#C9A84C]/40"
        />
      )}
            ...
            >
              <div className="flex items-center gap-3 mb-3">
                ...
                <div className="min-w-0">
                  <p className="font-medium text-[#0F172A] text-sm truncate">{t.name}</p>
              ...
              <div className="border-t border-[#F1F5F9] pt-3 space-y-1.5">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="text-[#0F172A] font-mono">{t.virtualAccountNumber}</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-[#64748B]">Bank</span>
                  <span className="text-[#0F172A]">{t.bankName}</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-[#64748B]">Paid this cycle</span>
                  <span className="text-[#0F172A] font-medium">{formatNaira(t.currentCycle.paid)}</span>
                </div>
              </div>
            </Link>
          ))}
