...
export default function PageBanner({ image, height = "h-32", title, subtitle, children }) {
  return (
    <div className={`relative w-full ${height} bg-[#0F172A] overflow-hidden shrink-0`}>
      {image && (
        <img src={image} alt="" className="absolute inset-0 w-full h-full object-cover opacity-10" />
      )}
